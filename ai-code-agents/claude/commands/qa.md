...
Delegate a structured code review of `$ARGUMENTS` to a review subagent.

Launch a review subagent (use `antipattern-sniffer`; if the rubric below needs
tools it lacks, use `general-purpose` instead) with the following prompt,
...
Relay the subagent's report back to the user without re-reviewing the code yourself.
