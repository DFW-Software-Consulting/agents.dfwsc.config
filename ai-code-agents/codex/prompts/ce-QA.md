...
- Capture diffs:
  - Run `git diff --name-status $START..$END`.
  - Run `git diff --stat $START..$END`.
  - Run `git log --oneline $START..$END`.
- Extract changed files (focus on src/ and tests/), group by module/package.
- For each changed file:
  - Run `git diff $START..$END -- $FILE` for evidence context only.
...
- Map **each changed public function/endpoint** to test coverage:
  - Run `pytest -q` and, if useful, `coverage run -m pytest && coverage report --format=markdown`; report results only.
