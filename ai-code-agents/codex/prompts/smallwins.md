...
* Project roots: inspect `README*`, `CONTRIBUTING*`, `.editorconfig`, `.gitignore`, `pyproject.toml`, `package.json`, `Cargo.toml`, and `tsconfig.json` when present
* Lint/setup hints:

  * Python: inspect `pyproject.toml`, `ruff.toml`, and `mypy.ini` when present
  * TS/JS: inspect `biome.json`, `eslint*`, `.eslintrc*`, and `prettier*` when present
  * Rust: inspect `Cargo.toml` and `rust-toolchain*` when present
