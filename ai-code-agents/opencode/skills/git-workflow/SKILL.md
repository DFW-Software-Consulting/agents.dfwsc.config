...
4. **Always use `gh` CLI** for PRs and issues — never ask the user to do it manually.
...
gh auth status 2>&1
# If not authenticated, output: "Run 'gh auth login' first" and stop.
...
If `gh` is not installed or not authenticated, fall back to generating the filled template for manual use. Never block entirely — always provide the content.
...
If `gh` is not available, output the filled template and the command to run manually.
...
| **Create PR** | Phases 1 → 5 (full lifecycle + PR creation) |
| **Create Issue** | Phase 6 only (detect template → fill → create) |
...
- If `gh` CLI is unavailable, always provide the filled template content so the user can proceed manually.
