...
When facts span multiple independent areas (e.g., API handlers, client components, services, schema), spawn parallel subagents to explore them simultaneously:

- Count the distinct areas from the facts; spawn one subagent per area (up to 4-5 in parallel)
- Use `subagent_type: "explore"` for surface-level area mapping, or `"codebase-analyzer"` when deep behavior tracing is needed
- Instruct each subagent with concrete questions tied to its area's facts
- After all return, synthesize findings into the plan

Subagent prompt template:

```
Explore the codebase to validate implementation paths for these facts:
[list facts relevant to this area]

Specifically:
- Find relevant files and trace the data flow
- Identify risks, constraints, or unknowns
- Check for existing patterns to follow
- Note files that will need modification

Report: file paths, patterns found, risks identified.
```
