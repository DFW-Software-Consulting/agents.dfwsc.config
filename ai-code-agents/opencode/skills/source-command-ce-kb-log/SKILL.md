---
name: "source-command-ce-kb-log"
description: "Creates a new KB entry with proper frontmatter and semantic directory placement"
---

# source-command-ce-kb-log

Use this skill when the user asks to run the migrated source command `ce-kb-log`.

## Command Template

# Log KB Entry

Create a structured knowledge base entry in the project's `.Codex/` directory.

This command is invoked from within a project. The `.Codex/` directory refers to the current project's directory, not this config directory.
...
Choose the most semantically correct directory under `.Codex/` (the user's project, not this config dir):
...
3. **Create subdirectory if needed** — `mkdir -p .Codex/<directory>` in the project root
...
**File:** `.Codex/debug_history/import-error-resolution.md`
